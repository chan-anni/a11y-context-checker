...
async function run() {
  try {
    ...
    // Get a list of files actually changed in this PR to avoid API path errors
    ...
    if (octokit && isPullRequest) {
      ...
      changedFiles = new Set(prFiles.map(f => f.filename));
      core.info(`📋 Files modified in this PR: ${Array.from(changedFiles).join(', ')}`);
    }
      ...
      rules.forEach(rule => {
        ...
        if (Array.isArray(issues)) {
          issues.forEach(issue => {
            ...
            // Clean up the issue relativePath format
            issue.relativePath = issue.relativePath.replace(/\\/g, '/');
          });
    ...
    if (allViolations.length > 0) {
      for (const issue of allViolations) {
        // Core Guard: If we are in a PR, skip files that aren't part of the diff
        if (isPullRequest && !changedFiles.has(issue.relativePath)) {
          core.info(`⏭️ Skipping PR review comment for unmodified file: ${issue.relativePath}`);
          continue;
        }
...
\`\`\`
        ...
        if (octokit && isPullRequest) {
          ...
          core.info(`✅ Live comment posted to line ${issue.startLine} in ${issue.relativePath}`);
        } else {
